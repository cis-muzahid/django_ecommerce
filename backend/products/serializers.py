...
class CategorySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Category
        fields = [
            'id', 'name', 'parent_category', 'parent_categories', 'subcategories', 
    ...
    def get_parent_categories(self, obj):
        """Get all ancestor categories for breadcrumb navigation"""
        categories = []
        current = obj
        while current is not None:
            if current.parent_category is not None:
                categories.append(current.parent_category.name)
...
class ProductReviewSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ProductReview
        fields = [
            'id', 'review', 'title', 'comment', 'product', 'user', 'user_name',
...
class ProductDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'review_count', 'available_variants', 'main_image'
        ]
        read_only_fields = ['id', 'user', 'created_at', 'updated_at']
    ...
    def get_main_image(self, obj):
        """Get the first product attribute image"""
        attribute = ProductAttribute.objects.filter(
            product=obj, product_image__isnull=False, is_delete=False
        ).first()
        if attribute and attribute.product_image:
